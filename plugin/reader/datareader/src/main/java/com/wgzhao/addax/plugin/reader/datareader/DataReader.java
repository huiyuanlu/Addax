            ...
            extends Reader.Job
    {
        ...
        public void init()
        {
            ...
            if (null == sliceRecordCount) {
                throw AddaxException.asAddaxException(DataReaderErrorCode.REQUIRED_VALUE,
                ...
                if (StringUtils.isBlank(typeName)) {
                    ...
                    if (Type.DATE.name().equalsIgnoreCase(typeName)) {
                        ...
                        if (notAssignDateFormat) {
                            eachColumnConfig.set(Key.DATE_FORMAT, DataConstant.DEFAULT_DATE_FORMAT);
                        }
                    }
        ...
        private void parseMixupFunctions(Configuration eachColumnConfig)
        {
            ...
            String columnRandom = eachColumnConfig.getString(DataConstant.RANDOM);
            String columnIncr = eachColumnConfig.getString(DataConstant.INCR);
            if (StringUtils.isBlank(columnRandom) && StringUtils.isBlank(columnIncr)) {
                eachColumnConfig.getNecessaryValue(Key.VALUE, DataReaderErrorCode.REQUIRED_VALUE);
            }
            if (StringUtils.isNotBlank(columnIncr)) {
                ...
                if ("long".equals(dType)) {
                    ...
                    if (!columnIncr.contains(",")) {
                        ...
                        eachColumnConfig.set(DataConstant.INCR, columnIncr);
                    }
                    // validate value
                    try {
                    ...
                    if (fields.length == 1) {
                        eachColumnConfig.set(DataConstant.INCR, columnIncr.trim() + ",1,d");
                    }
                        ...
                        eachColumnConfig.set(DataConstant.INCR, fields[0].trim() + "," + fields[1].trim() + ",d");
                    }
                    else {
                        ...
                        eachColumnConfig.set(DataConstant.INCR, fields[0].trim() + "," + fields[1].trim() + "," + unit);
                    }
                }
                else {
                    throw AddaxException.asAddaxException(
                ...
                this.originalConfig.set(DataConstant.HAVE_INCR_FUNCTION, true);
            }
            // 三者都有配置
            if ((StringUtils.isNotBlank(columnRandom) || StringUtils.isNotBlank(columnIncr)) && StringUtils.isNotBlank(columnValue)) {
                LOG.warn("您配置了datareader常量列(value:{})和随机混淆列(random:{})或递增列(incr:{}), 常量列优先",
                        columnValue, columnRandom, columnIncr);
                if (StringUtils.isNotBlank(columnRandom)) {
                    eachColumnConfig.remove(DataConstant.RANDOM);
                }
                if (StringUtils.isNotBlank(columnIncr)) {
                    eachColumnConfig.remove(DataConstant.INCR);
                }
            }
            if (StringUtils.isNotBlank(columnRandom)) {
                ...
                String typeName = eachColumnConfig.getString(Key.TYPE);
                if (Type.DATE.name().equalsIgnoreCase(typeName)) {
                    String dateFormat = eachColumnConfig.getString(Key.DATE_FORMAT, DataConstant.DEFAULT_DATE_FORMAT);
                    try {
                        SimpleDateFormat format = new SimpleDateFormat(
                                eachColumnConfig.getString(Key.DATE_FORMAT, DataConstant.DEFAULT_DATE_FORMAT));
                ...
                eachColumnConfig.set(DataConstant.MIXUP_FUNCTION_PARAM1, param1Int);
                eachColumnConfig.set(DataConstant.MIXUP_FUNCTION_PARAM2, param2Int);
                if (split.length == 3) {
                    int scale = Integer.parseInt(split[2].trim());
                    eachColumnConfig.set(DataConstant.MIXUP_FUNCTION_SCALE, scale);
                }
...
//            this.haveMixupFunction = readerSliceConfig.getBool(DataConstant.HAVE_MIXUP_FUNCTION, false);
            this.haveIncrFunction = readerSliceConfig.getBool(DataConstant.HAVE_INCR_FUNCTION, false);
        }
                ...
                throws Exception
        {
            ...
            String columnRandom = eachColumnConfig.getString(DataConstant.RANDOM);
            String columnIncr = eachColumnConfig.getString(DataConstant.INCR);
            long param1Int = eachColumnConfig.getLong(DataConstant.MIXUP_FUNCTION_PARAM1, 0L);
            long param2Int = eachColumnConfig.getLong(DataConstant.MIXUP_FUNCTION_PARAM2, 1L);
            int  scale = eachColumnConfig.getInt(DataConstant.MIXUP_FUNCTION_SCALE, -1);
            ...
            if (isColumnMixup) {
                ...
                if (columnType == Type.LONG) {
                    ...
                    if (currVal == null) {
                        String datePattern = eachColumnConfig.getString(Key.DATE_FORMAT, DataConstant.DEFAULT_DATE_FORMAT);
                ...
                switch (columnType) {
                    ...
                    case DATE:
                        SimpleDateFormat format = new SimpleDateFormat(eachColumnConfig.getString(Key.DATE_FORMAT, DataConstant.DEFAULT_DATE_FORMAT));
